...
module.exports = (bot) => {
  bot.onText(/\/draw (.+)/, async (msg, match) => {
    ...
    if (!query) {
      return bot.sendMessage(chatId, "يرجى إدخال نص أو استفسار.");
    }

    bot.sendMessage(chatId, "مرحبًا بك في أمر الرسم، يُرجى اختيار رقم الستايل المفضل:\n1. سينمائي\n2. فوتوغرافي\n3. أنيمي\n4. مانغا\n5. فن رقمي\n6. بيكسل آرت\n7. فن الخيال\n8. نيون بانك\n9. نموذج ثلاثي الأبعاد");

    bot.once('text', async (msg) => {
      const chatId = msg.chat.id;
      const styleChoice = parseInt(msg.text);
